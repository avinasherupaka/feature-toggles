...
export const TOGGLE_NONPROD_FEATURE_TOGGLE = 'TOGGLE_NONPROD_FEATURE_TOGGLE';
export const TOGGLE_PROD_FEATURE_TOGGLE = 'TOGGLE_PROD_FEATURE_TOGGLE';
...
export function toggleFeature(enable, env, name) {
    debug('Toggle feature toggle ', name);
    return dispatch => {
        dispatch(requestToggleFeatureToggle(enable, env, name));
    };
}
...
function receiveFeatureToggles(json) {
    debug('received feature toggles', json);
    return {
...
export function requestToggleFeatureToggle(enable, env, name) {
    return dispatch => {
        dispatch({ type: START_UPDATE_FEATURE_TOGGLE });
        return api
            .toggle(enable, env, name)
            .then(() =>
                dispatch({ type: env === 'np' ? TOGGLE_NONPROD_FEATURE_TOGGLE : TOGGLE_PROD_FEATURE_TOGGLE, env, name })
            )
